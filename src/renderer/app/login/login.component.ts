...
@Component({
    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-900 text-white">
      <div class="bg-gray-800 p-8 rounded-lg shadow-xl w-96 border border-gray-700">
        <h2 class="text-2xl font-bold mb-6 text-center text-blue-400">NalamVault</h2>
        ...
        <form (ngSubmit)="onLogin()">
          ...
          >
            {{ isLoading ? 'Unlocking...' : 'Unlock Safe' }}
          </button>
        </form>
        
        <p class="mt-4 text-xs text-gray-500 text-center">
          Local-Only. Zero-Knowledge. <br>
          <span class="text-red-400">If you lose this password, data is gone.</span>
        </p>
      </div>
    </div>
  `,
...
export class LoginComponent {
    ...
    async onLogin() {
        ...
        try {
            const result = await window.electron.login(this.password);
            ...
            this.ngZone.run(() => {
                ...
                if (result.success) {
                    ...
                    if (result.error === 'INVALID_PASSWORD') {
                        this.error = 'Incorrect Password. Data remains encrypted.';
                    } else {
            ...
            this.ngZone.run(() => {
                this.isLoading = false;
                this.error = 'Communication error with safe.';
                console.error(e);
            });
        }
    }
